...
(function ($) {
   $.fn.square1 = function (options) {
      ...
      $(".image_wrapper", $this).each(function () {
         $img = $(this).find("img");
         $(this).attr("scale-from", $img.attr("scale-from")).attr("scale-from-mobile", $img.attr("scale-from-mobile")).attr("space", $img.attr("space"));
      });
      ...
      if (_this.settings["animation"] == "slide") {
         ...
         $(".image_wrapper", $this)
            .first()
            .clone()
            .removeClass("image_wrapper")
            .addClass("wrap_placeholder")
            .css({ left: $(".image_wrapper", $this).length * 100 + "%" })
            .appendTo($(".slides_container", $this));
      }
      ...
      function next_image() {
         ...
         if (filter_gallery) {
            next_slide_index = $(".current_slide", $this)
               .nextAll('.image_wrapper[space*="' + filter_gallery + '"]')
               .index();
            if (
               next_slide_index < 0 &&
               $(".image_wrapper", $this)
                  .first()
                  .is('.image_wrapper[space*="' + filter_gallery + '"]')
            )
               next_slide_index = $(".image_wrapper", $this).first().index();
            if (next_slide_index < 0)
               next_slide_index = $(".image_wrapper", $this)
                  .first()
                  .nextAll('.image_wrapper[space*="' + filter_gallery + '"]')
                  .index();
         }
      ...
      function prev_image() {
         ...
         if (filter_gallery) {
            prev_slide_index = $(".current_slide", $this)
               .prevAll('.image_wrapper[space*="' + filter_gallery + '"]')
               .index();
            if (
               prev_slide_index < 0 &&
               $(".image_wrapper", $this)
               ...
               prev_slide_index = $(".image_wrapper", $this).last().index();
            if (prev_slide_index < 0)
               prev_slide_index = $(".image_wrapper", $this)
                  .first()
                  .prevAll('.image_wrapper[space*="' + filter_gallery + '"]')
                  .index();
         }
            ...
            if (_this.settings["fill_mode"] == "cover") {
               ...
               if (image_num > $(".current_slide", $this).index()) {
                  ...
                  $(".image_wrapper", $this)
                     ...
                     .fadeIn(_this.settings["transition_time"], function () {
                        $(".image_wrapper:not(.current_slide)", $this).hide();
                     });
               }
      ...
      function update_caption() {
         ...
         $(".square1_caption", $this).fadeOut(_this.settings["transition_time"], function () {
            setTimeout(function () {
               // Add a delay so that new image can load before getting new caption
               $(".square1_caption", $this)
                  .html($(".current_slide img", $this).attr("alt"))
                  .fadeIn(_this.settings["transition_time"] - 200);
            }, 10);
         });
      }
