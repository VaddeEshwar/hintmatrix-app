    ...
    action_array.forEach(function(itm, idx){
        ...
        if(dp_code){
            ...
            if(jQuery('tr[data-slug-pair="'+slug+'"]').length > 0){
                _a = jQuery('a[data-slug="'+slug+'"]');
                _a.removeClass("cls_show_ans_box").removeClass("link-dark");
...
if (jQuery(".cls_lbl").parent().find(".tooltiptext")) {
    jQuery(document).on("click", "body", function () {
        ...
        let nextTarget = jQuery(".cls_lbl").parent().find(".tooltiptext").parent().parent().parent();
...
total_lbl += jQuery("a.cls_ele_adj").length;
