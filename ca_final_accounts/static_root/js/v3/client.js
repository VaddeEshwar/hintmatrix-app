function calculate_profit_loss() {
   let tr_dr, tr_cr, loss, profit, pl_dr, pl_cr, bl_la, tr_dr_dom, tr_cr_dom, pl_dr_dom_loss, pl_dr_dom_profit, pl_cr_dom_loss, pl_cr_dom_profit, bs_ls_dom_profit, bs_ls_dom_loss;
   tr_dr = 0;
   tr_cr = 0;
   loss = 0;
   profit = 0;
...
function add_answered_row(slug, action_array, particular_name, amount, order_by, pair) {
   ...
   action_array.forEach(function (itm, idx) {
      ...
      _a = jQuery('[data-slug="' + slug + '"]')
         .removeClass("cls_show_ans_box")
...
$(document).click(function (e) {
   if (!$(e.target).parents().is(".tooltip-one")) {
      $(".cls_show_ans_box").removeClass("open");
   }
});
...
function answer_failure_cb(res, _jObj) {
   if (res.data[0].help) {
      _help = '<abbr title="' + res.data[0].help[0] + '">help</abbr>';
