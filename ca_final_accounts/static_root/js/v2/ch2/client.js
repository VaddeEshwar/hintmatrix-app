...
function add_answered_row(slug, action_array, particular_name, amount, order_by, pair){
    ...
    action_array.forEach(function(itm, idx){
        ...
        let dp_code = jQuery('[data-slug="'+slug+'"]').attr("data-particular-code");
        if(dp_code){
            // for 2 places same particulat moved then only text-strickoff.
            if(jQuery('[data-particular-pair="'+dp_code+'"]').length > 0){
