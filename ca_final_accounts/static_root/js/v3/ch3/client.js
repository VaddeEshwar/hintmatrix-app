...
function make_dr_narration(tbl_name){
    return tbl_name + " " + ".".repeat(35-tbl_name.length-3)+" Dr";
}
...
function update_result_dummy(){
    let total_score = 0;
    jQuery(".cls_element_score").each(function(idx, itm){
        total_score += parseFloat(itm.innerText);
    });
    let tot_ele = jQuery("a.cls_lbl.text-strikethrough").length*2;
    if(tot_ele < 1){
        tot_ele = 1;
    }
